...
class Dual_Video_Dialog(QDialog):
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        frame_info_layout, video_label_layout = self._build_ui_layouts()
...
class Track_Correction_Dialog(Dual_Video_Dialog):
    def __init__(
        ...
        self.nonempty_local = set(
            np.where(np.any(~np.isnan(pred_data_array[self.frame_list]), axis=(1, 2)))[0].tolist()
        )
